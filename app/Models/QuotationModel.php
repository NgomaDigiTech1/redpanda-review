...
class QuotationModel {
    ...
    function __construct() {
        ...
        $this->collection = $database->rp_quotation;
    }

    function getQuotes() {
        try {
            $cursor = $this->collection->find([]);
            $quotes = $cursor->toArray();
            return $quotes;
        } catch(\MongoDB\Exception\RuntimeException $ex) {
            show_error('Error while fetching quote: ' . $ex->getMessage(), 500);
        }
    }
    ...
    function getQuote($id) {
        try {
            ...
            show_error('Error while fetching quote with ID: ' . $id . $ex->getMessage(), 500);
        }
    }    
